...
export default function Header() {
	...
	return (
		<header className="absolute inset-x-0 top-0 z-50 bg-background ">
			<nav
				className="flex items-center justify-between p-3 py-5 lg:py-3 lg:px-8"
				aria-label="Global"
			>
			...
			>
				...
				<DialogPanel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-background px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-neutral-900/10">
							...
							<div className="flex flex-col space-y-2 py-6">
								{navigation.map((item) => (
									<Link
										key={item.name}
										href={item.href}
										className="-mx-3 block rounded-lg px-3 py-2 text-base leading-7 text-neutral-900 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-muted"
									>
										{item.name}
									</Link>
								))}
