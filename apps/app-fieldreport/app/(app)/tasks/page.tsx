...
import { format, isToday as checkIfToday, isTomorrow as checkIfTomorrow } from "date-fns";
import {
    TableHead,
    TableRow,
    TableHeader,
...
export default function TaskTable() {
    ...
    if (localTasks.length === 0) return <NoDataContextCard title="No tasks found" description='You have no tasks to display.' />
    if (localTasks.every((task) => task.sub_tasks.every((subTask) => subTask.completed))) {
        return <NoDataContextCard title="You're all caught up!" description='Enjoy your day, you have no pending tasks.' />
    }
    ...
    const renderTaskTable = (category: { date: string, tasks: ITask[] }) => {
        ...
        return (
            <div key={category.date} className="mb-8">
                <h2 className="text-lg font-semibold mb-4">{isUpNext ? "Up next" : isTomorrow ? "Tomorrow" : format(date, "dd MMM")}</h2>
